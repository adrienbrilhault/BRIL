...
#'   providing the proportion of unasnigned samples from `data` below which the
...
#'   an iteration. `FALSE` (default) will let that last iteration proceed before
#'   exiting
...
#'   following values:
#'   \item{`call`}{Parameters of the call to the function (i.e. `data`,
...
#'   \item{`iterations`}{A list of each iteration, containing the two filtering
#'   procedures performed: `filteringUnimodal` and `filteringNormal` (both being
#'   S3 object of class `BRIL.Filtering`, see [filter_outliers()])}
#'   \item{`nbClusters`}{Number of groups encountered (one per iteration)}
#'   \item{`labels`}{Labels of the groups encountered (cooresponding to the
...
#'   \item{`clustersSizes`}{Array with the number of sample in each group}
#'   \item{`mainCluster`}{Index of the group identified as main mode}
#'
#' @seealso [plot.BRIL()], [print.BRIL()], [median_rec()], [median_mv()], [filter_outliers()], [depth_values()]
#'
#' @examples
#'
#' XY <- rbind(
...
bril <- function(data, maxIterations = NULL, minUnassigned = 0.1, method = "Projection", alpha = 0.5,
  ...
  while ((maxIterations == 0 || iterationNB <= maxIterations) &&
         (length(which(output$labels == 0)) > max(minSampleSize, minUnassigned * nrow(data)))) {
    unassginedIndices <- which(output$labels == 0)

    ## Bootstrap : compute first estimate

    bootstrapEstimate <- median_rec(data[unassginedIndices, ], method = method, alpha = alpha, warnings = warnings)$median

    ## Refine estimate

    # Filter furthest outliers until reaching an unimodal subset
    filteringUnimodal <- filter_outliers(
      data[unassginedIndices, ],
    ...
    unimodalIndices <- unassginedIndices[filteringUnimodal$selected]
    ...
    if (recenterAfterUnimodal == TRUE) {
      bootstrapEstimate <- median_rec(data[unimodalIndices, ], method = method, alpha = alpha, warnings = warnings)$median
    }
...
#' @param showClusters Logical value. Used with contents="scatterplot", to show
#'   or not the different clusters
#' @param showMode Logical value. Used for contents="clusters", to show or not
#'   the main mode to show or not the different clusters
#' @param col default color of samples
#' @param colMode Color of the mode when contents="scatterplot" (if showMode
#'   is TRUE)
#' @param colClusters List (or array) of colors (length must be at least equal
#'   to the number of clusters identified by the function [bril()], i.e.
#'   `x$nbClusters)
#' @param iterationsIndices Numerical value or array of numerical values, used
#'   with contents="iterations", providing the indices of the iterations to be
#'   ploted. If more than one iteration is requested, an interactive menu in the
#'   console will be used for the selection. 0 or NULL (default) will include
#'   all the iterations. Values that are negative or superior to the number of
#'   iterations performed by the execution [bril()] will be ignored
#' @param iterationsOptions List of additional parameters to be passed to the
#'   `plot.BRIL.Filtering` function when contents="iterations" is selected (see
#'   [plot.BRIL.Filtering] for details). \emph{Example: iterationsOptions =
#'   list(Xlab=NA, Ylab=NA, contents=c("p.values", "scatterplot"))}
...
#'
#' @seealso [bril()], [print.BRIL], [filter_outliers()], [print.BRIL.Filtering()]
#'
#' @export
#'
plot.BRIL <- function(x, contents = "plot", showClusters = TRUE, showMode = TRUE,
  ...
  if (max(iterationsIndices) > x$nbClusters) {
    warning("Only ", x$nbClusters, " iterations, values above disconsidered")
    iterationsIndices <- iterationsIndices[iterationsIndices <= x$nbClusters]
  }
  if (min(iterationsIndices) < 1) {
    warning("Paramenter `iterations` can not take negative values")
  ...
  if (any(sapply(c("scater", "scatter", "plot"), grepl, contents))) {
    ...
    if (showMode == TRUE) {
      graphics::points(x$mode[1], x$mode[2],
                       col = colMode, pch = 4, lwd = 2
      )
    }
    ...
    if (length(iterationsIndices) == 1) {
      ...
      do.call(plot, utils::modifyList(
        append(defaultOptions, list(x = x$iterations[[iterationsIndices]]$filteringUnimodal,
      ...
      do.call(plot, utils::modifyList(
        append(defaultOptions,list(x = x$iterations[[iterationsIndices]]$filteringNormal,
                                   colSelection = myColours[iterationsIndices])),
        extraOptions)
      )
      ...
      # present menu
      choice <- utils::menu(options, graphics = FALSE, title = "Plot Selection")
      while (choice != 0) {
        if (choice %% 2 == 1) {
          do.call(plot, utils::modifyList(
            append(defaultOptions, list(x = x$iterations[[(choice + 1) %/% 2]]$filteringUnimodal,
                                        colSelection = myColours[((choice + 1) %/% 2)])),
            extraOptions)
          )
        } else {
          do.call(plot, utils::modifyList(
            append(defaultOptions,list(x = x$iterations[[(choice + 1) %/% 2]]$filteringNormal,
                                       colSelection = myColours[((choice + 1) %/% 2)])),
            extraOptions)
          )
        }
